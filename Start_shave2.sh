...
# About HaplotypeCaller errors:
# Sometimes, HaplotypeCaller output an error like:
#
#  """A USER ERROR has occurred: Argument emit-ref-confidence has a bad value: Can only be used in single sample mode currently. 
#     Use the --sample-name argument to run on a single sample out of a multi-sample BAM file.""""
#
# This is an error about missing or undefined read groups on your BAM files. 
# As detailed in the FAQs about input requirements, GATK expects all read groups appearing in the read data to be specified in the file header, 
# and will fail with an error if it does not find that information (whether there is no read group information in the file, or a subset of reads 
# do not have read groups).

# Typically you should add read group information when you perform the original alignments (with e.g. BWA, which has an option to do so). So what 
# do you do if you forgot to do that, and you don't want to have to rerun BWA all over again?
